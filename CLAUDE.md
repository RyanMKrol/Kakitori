...
- **Kakitori** (書き取り) is a **native SwiftUI iOS/iPadOS app (minimum deployment target iOS 18,
  Swift 6 strict concurrency)** for learning to **write Japanese characters by hand**: a
  spaced-repetition loop (SM-2 derived) where every card ends with the user physically writing the
  target on a **PencilKit** canvas, self-grading Again/Hard/Good/Easy against the revealed answer.
  Content arrives as Anki `.apkg` decks (from the companion `anki-builder` pipeline); progress is
  stored on-device in **SwiftData**. Universal app, **iPad-first layout** (side-by-side prompt +
  canvas), iPhone fully designed. Offline-first: no accounts, no server, no third-party
  dependencies in v1.
- **The design docs in `docs/` are the product spec** — `docs/01-overview.md` through
  `docs/07-roadmap.md` cover the product, the SRS algorithm (exact SM-2 math), the `.apkg`
  content contract, the architecture, and the UI design. The interactive HTML prototypes in
  `reference/` are the visual/behavioral spec for v1.
...
For Kakitori specifically: unit tests use an **in-memory SwiftData ModelContainer**, never the app's
real `Application Support/Kakitori/` store, and importer tests unzip fixtures into a temp directory.
...
- **Stack:** Swift 6 (strict concurrency) / SwiftUI, iOS 18 minimum, universal iPhone+iPad,
  built with Xcode (`xcodebuild`). First-party frameworks only in v1 — `SwiftUI`, `SwiftData`,
  `PencilKit`, `AVFoundation` (audio), `SQLite3` (the system C library, for reading
  `collection.anki2` inside imported `.apkg` decks). **No SwiftPM dependencies** (GRDB is the
  sanctioned escape hatch ONLY if raw SQLite becomes untenable — a deliberate decision to record
  in `.harness/custom/docs/LIMITATIONS.md` if taken).
- **The `Kakitori.xcodeproj` is GENERATED by XcodeGen from `project.yml` and is git-ignored — never
  hand-edit it or `Sources/Info.plist`.** New `.swift` files just go under `Sources/` / `Tests/` and
  are in the target on the next `xcodegen generate`. This is what lets the autonomous loop grow the
...
- **Definition of Done (mirrored verbatim in `.github/workflows/ci.yml`, CI is the authoritative gate):**
  ...
  - **Test + Build:** `xcodebuild test -project Kakitori.xcodeproj -scheme Kakitori -destination '…' CODE_SIGNING_ALLOWED=NO` — `xcodebuild test` compiles first, so the build step is folded in. Simulator builds need **no code signing**. CI resolves the simulator name dynamically (`.github/actions/resolve-simulator`, iPad-preferred) so a runner dropping a model doesn't hard-fail the gate.
- **Local runs target a DEDICATED, uniquely-named simulator — `Kakitori-Sim` — never a generic model
  name.** It is an **iPad** (Kakitori is iPad-first). `tools/loop_sim.sh` idempotently ensures it
  exists (reuses it, else creates it on the newest installed iOS runtime) and prints its UDID; prefix
  local test commands with `./tools/loop_sim.sh >/dev/null &&` (the committed `LOCAL_DOD` does). A
  unique NAME is the collision guard — several autonomous iOS loops share this Mac (Scout, Sprout,
  Enough, Basket), and two loops sharing a generic `name=iPhone 16` fight over the foreground app and
  reset each other's sims mid-test. **Pinning must cover EVERY surface that names a destination**
  (the `LOCAL_DOD` gate, `build_run.sh`, the build preamble, both CLAUDE.mds, task specs, and any NEW
...
- **Testing convention: XCTest** (not Swift Testing — for consistency with the team's sibling
  ...
  `SM2Scheduler` (every grade × state transition, clamps, fuzz bounds, previews), `SessionQueue`
  (limits, ordering, sub-day re-entry), `TargetSegmenter` (kana/kanji/mixed/punctuation tables), and
  `ApkgImporter` (fixture `.apkg` round-trips, re-import idempotence). All tests run against an
  **in-memory SwiftData container** and a **temp media directory** — never the app's real store.
  Any "current time" read in scheduler/streak/stats code comes from an **injected clock**
  (`AppClock.now` / a `now:` parameter), never a bare `Date()` / `.now`, so day-rollover (4 AM) and
  interval math are deterministic under test. Audio (`AVAudioPlayer` / `AVSpeechSynthesizer`) sits
  behind a protocol with a scripted fake — tests never do real playback. **UI-test house rules:**
  query by `accessibilityIdentifier` (never display copy — the UI is bilingual Japanese/English);
  use bounded waits; never `sleep()`-poll.
- **Keep the DoD tooling off the vendored harness tree.** SwiftFormat and SwiftLint only scan
  `.swift` files, and none live under `.harness/` — so no ignore file is needed today. But if you
  ever add a whole-repo tool that touches Markdown/JSON (a docs linter, a Prettier pass), exclude
  `.harness/**` and the harness-authored root prose (`CLAUDE.md` / `README.md`) from it, or CI goes
  red out of the box and a build reformats out-of-scope files. Any task that reconfigures a
  whole-repo tool must keep that exclusion intact.
...
- Tasks marked **🔒 needs-human** require the user (credentials, provisioning, anything
  spending real money or touching production — or, here, anything needing **real Apple Pencil
  hardware feel**, like the M1 canvas ink-tuning gate). Do not attempt the human-gated portion
