    ...
    @MainActor final class SessionScaffoldRenderTests: XCTestCase {
        ...
        func testSessionViewRendersWithThreeCards() throws {
            ...
            makeReviewNote(target: "り", deck: deck, context: context)
            makeReviewNote(target: "う", deck: deck, context: context)
            ...
            let renderer = ImageRenderer(content: sessionView)
            renderer.scale = 2

            guard let uiImage = renderer.uiImage else {
                XCTFail("Failed to render SessionView to UIImage")
                return
            }
            ...
            let pngPath = screenshotsDir + "/T030-session.png"
